<template>
  <div class="page">
    <div class="top">
      ...
      <!-- <span class="top-text">卡号：{{ user.cardNumber }}</span> -->
      <!-- <span class="top-text">可圈存金额：{{ user.surplus }}元</span> -->
    ...
    <div class="table_parent">
      ...
      >
        ...
        <el-table-column label="支付来源" class-name="tc_4">
          <template #default="scope">
            <div class="table-text">{{ scope.row.paySource===1?'app':'终端机' }}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="是否圈存" class-name="tc_6">
          <template #default="scope">
            <div class="table-text">{{ scope.row.entrapStatus===1?'是':'否' }}</div>
          </template>
        </el-table-column>
...
function query() {
  ...
  API.queryUserPayRecord(data)
    .then((res: any) => {
      ...
      total.value = Math.ceil(res.result.total/pageSize.value);
    })
    .catch((err) => {});
}
...
function next() {
  if (current.value < pageSize.value) {
    current.value = current.value + 1;
  }
  query()
}
